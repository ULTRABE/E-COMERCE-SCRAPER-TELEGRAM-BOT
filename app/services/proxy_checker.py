...
class ProxyChecker:
    ...
    async def _fraud_ok(self, session: aiohttp.ClientSession, ip: str) -> bool:
        ...
        try:
            async with session.get(url, timeout=self.timeout) as r:
                ...
                row = data.get(ip, {})
                risk = int(float(str(row.get("risk", "100"))))
                is_proxy = str(row.get("proxy", "yes")).lower()
                return risk < 20 and is_proxy != "yes"
        except Exception:
            return False

    async def _check_one(self, session: aiohttp.ClientSession, proto: str, proxy: str):
        async with self.sem:
            proxy_url = f"{proto}://{proxy}"
            try:
                async with session.get("https://api.ipify.org?format=json", proxy=proxy_url, timeout=self.timeout) as r:
                    if r.status != 200:
                        return None
                    ip = (await r.json()).get("ip")
                    if not ip or not await self._fraud_ok(session, ip):
                        return None
        ...
        async with aiohttp.ClientSession(timeout=self.timeout) as session:
            ...
            for fut in asyncio.as_completed(tasks):
                ...
                if result:
                    ...
                    filtered += 1
                if progress_cb and (done % 100 == 0 or done == len(tasks)):
